...
void *touchThFunc(void* args)
{
	...
	while (1)
	{
		...
		else if ( (stEvent.type == EV_KEY) && (stEvent.code==BTN_TOUCH) )
		{
			...
			msgsnd(msgID, &sendMsg, sizeof (BUTTON_MSG_T2) - sizeof (long int), 0);
		}
		/*
...
int touchInit(void)
{
	...
	BUTTON_MSG_T2 trashCan;
	while (	msgrcv (msgID, &trashCan, sizeof(BUTTON_MSG_T2) - sizeof(long int), 0, IPC_NOWAIT) >= 0	)
	{
